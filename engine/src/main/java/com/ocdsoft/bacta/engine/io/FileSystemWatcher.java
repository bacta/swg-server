...
public final class FileSystemWatcher {
    ...
    public void start() {
        if (watching) {
            log.error("This instance is already watching. Create a new instance.");
            return;
        }

        new Thread(() -> {
            try (final WatchService watcher = FileSystems.getDefault().newWatchService()) {
                log.debug("Watching {} for file changes.", filePath.toString());
                ...
                watching = false;
                log.debug("Not watching for file changes anymore.");
            } catch (final Exception ex) {
                log.error("Unexpectedly stopped watching: {}", ex.getMessage());
            }
        }).start();
    }
     ...
     */
    public void stop() {
        log.debug("Requested to stop watching for file changes.");
        watching = false;
    }
}
