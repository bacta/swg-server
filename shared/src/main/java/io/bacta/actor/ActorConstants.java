package io.bacta.actor;

public class ActorConstants {
    public static final String ACTOR_SYSTEM_NAME = "galaxy";

    public static final String LOGIN_SUPERVISOR = "login";
    public static final String GAME_NETWORK_MESSAGE_RELAY = "GameNetworkMessageRelay";

    public static final String PUBLISHER = "publisher";
    public static final String GALAXY_SCENE_SUPERVISOR = "scene";
    public static final String CONNECTION_SERVER = "connection";

    public static final String CHAT_SUPERVISOR = "chat";

    public static final String PLAYER_CREATION_SERVICE = "playerCreation";
    public static final String NAME_VALIDATION_SERVICE = "nameValidation";
}
