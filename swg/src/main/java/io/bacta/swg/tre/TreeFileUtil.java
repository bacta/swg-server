...
final class TreeFileUtil {
    ...
    public static int makeId(final String id) {
        final byte[] bytes = id.getBytes();
        return ((bytes[0] << 24 | bytes[1] << 16 | bytes[2] << 8 | bytes[3]));
    }

    public static String getNameForId(final int id) {
        return new String(ByteBuffer.allocate(4).putInt(id).array());
    }

    public static String readNullTerminatedString(final ByteBuffer buffer) {
        return readNullTerminatedString(buffer, buffer.position());
    }

    public static String readNullTerminatedString(final ByteBuffer buffer, int offset) {
    ...
    public static void expand(ByteBuffer buffer, ByteBuffer dst, int compressor, int deflatedSize) {
        final byte[] src = new byte[deflatedSize];
        buffer.get(src);
